...
public class Main {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] ints : direction) {
                ...
                if (strLen > 0) {
                    ...
                    if (command == UP && (prev == 'D' || prev == 'U')) continue;
                    else if (command == DOWN && (prev == 'U' || prev == 'D')) continue;
                    else if (command == LEFT && (prev == 'R' || prev == 'L')) continue;
                    else if (command == RIGHT && (prev == 'L' || prev == 'R')) continue;
                }
                ...
                if (command == UP || command == DOWN) {
                    if (pop.blueY == pop.redY) {
                        if ((command == UP && pop.blueX < pop.redX) || (command == DOWN && pop.blueX > pop.redX)) {
                            ...
                            queue.offer(new N13460Board(redPosition[0], redPosition[1], bluePosition[0], bluePosition[1], pop.count + 1, pop.direction + cDirection[command]));
                        } else {
    ...
    static class N13460Board {
        ...
        public N13460Board(int redX, int redY, int blueX, int blueY, int count, String direction) {
            this.redX = redX;
            this.redY = redY;
            this.blueX = blueX;
            this.blueY = blueY;
